...
def fgsm_loss(model, criterion, inputs, labels, defense_args, return_preds = True):
    ...
    grads = torch.autograd.grad(loss, inputs, retain_graph=True, create_graph=False)[0]
    perturbed_data = fgsm_attack(inputs, grads, epsilon).detach()
...
def test_attack(model, test_loader, attack_function, attack_args):
    ...
    for data, target in test_loader:
        ...
        if attack_function == FGSM: 
            ...
            gradients = torch.autograd.grad(loss, data, retain_graph=False, create_graph=False)[0]
            perturbed_data = fgsm_attack(data, gradients, epsilon=attack_args[EPSILON])
            output = model(perturbed_data)
