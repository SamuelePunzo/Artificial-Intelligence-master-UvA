...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x * (torch.rsqrt(torch.mean(x**2, dim=-1, keepdim=True)+self.eps)) * self.weight
        return output
...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        seq_pos = torch.arange(0, T, device=xq.device).float()  # Shape: (T)
        inv_freq = self.inv_freq.to(xq.device)
        freqs = torch.outer(seq_pos, inv_freq)   # Shape: (T, dim // 2)
        ...
        h = xq.size(-1) // 2
        xq_rot = (xq * pos_cos) + (torch.cat((-xq[..., h:], xq[..., :h]), dim=-1) * pos_sin)
        xk_rot = (xk * pos_cos) + (torch.cat((-xk[..., h:], xk[..., :h]), dim=-1) * pos_sin)
    ...
    def forward(self, x):
        ...
        q = q.view(B, T, self.n_head, head_size).transpose(1, 2) # (B, nh, T, head_size)
        k = k.view(B, T, self.n_head, head_size).transpose(1, 2) # (B, nh, T, head_size)
        v = v.view(B, T, self.n_head, head_size).transpose(1, 2) # (B, nh, T, head_size)
        ...
        if self.use_flash_attn:
            ...
            att = torch.softmax(att, dim=-1)  # (B, nh, T, T)
            # Apply dropout to the attention probabilities
            att = self.attn_dropout(att)
            # Apply attention to the values
            y = att @ v # (B, nh, T, T) x (B, nh, T, head_size) -> (B, nh, T, head_size)
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer_norm_3 = RMSNorm(config.n_embd)
        ...
        for _ in range(max_new_tokens):
            ...
            if not do_sample:
                ...
                if top_k is not None:
                    partial_probs, _ = torch.topk(probs, top_k, dim=-1)
                    min_topk_probs = partial_probs[:, -1].unsqueeze(-1)

                    probs = torch.where(probs < min_topk_probs, torch.zeros_like(probs), probs)
